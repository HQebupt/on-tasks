...
// huangqiang added for ls -l
    CommandParser.prototype[lshuang] = function(data) {
        if (data.error) {
            return Promise.resolve({ source: 'lshuang', error: data.error });
        }
        try {
            console.log("===***=============== run here:lshuang" );
            var parsed = 'huangqiang ls';
            return Promise.resolve({ data: parsed, source: 'lshuang', store: true });
        } catch (e) {
            return Promise.resolve({ source: 'lshuang', error: e });
        }
    };
// huangqiang 'sudo ls -l /tmp'
CommandParser.prototype[lsTmp] = function(data) {
        if(data.error){
            return Promise.resolve({ source: 'lsTmp', error: data.error });
        }
        console.log("===***=============== response raw data [DEBUG]:" + data.stdout);
        console.log("===***=============== response raw data:" + JSON.stringify(data.stdout));
        ...
        try{
            ...
            var parsed = _.transform(lines, function(result, line) {
                ...
                if(!line) {
                    ...
                    var fields = line.split(' ').filter(Boolean);
                    console.log("================== line.split.filter fields:" + fields);
                    var flag = fields[0][0];
                    if(flag === 'd'){ 
                        var dir_name = fields[8].slice(0,-1);
                    ...
                    var permission = {
                        "user": fields[0].substr(1,3),
                        "group": fields[0].substr(4,3),
                        "others": fields[0].substr(7,3),
                    };
                   obj.permission = permission;
                  
                   obj.link_count = fields[1];
                   obj.owner = fields[2];
                   obj.group = fields[3];
                   obj.size = fields[4];
                   obj.modified_time = fields[5] + " " + fields[6] + " " + fields[7];
              }
              result.push(obj); 
            },[]);
            console.log("================== lines.parser parsed:" + JSON.stringify(parsed));
            return Promise.resolve({ data: parsed, source: 'lsTmp', store: true });

        }catch(e){
           return Promise.reject({ source: 'lsTmp', error: e });
        }
    };
